...
1. Bilateral flow data with three columns only (origin, destination and flow), see [here](https://github.com/gjabel/china-chord/blob/master/flow.csv) for the file used below
2. Region details used for plotting, see [here](https://github.com/gjabel/china-chord/blob/master/region.csv) for the file used below
...
To replace the labels with their Chinese names, as in the plot above, replace the code for `s` in the `circos.track()` function to:
...
You might also need to add `family = "GB1"` in the `pdf()` function for Chinese characters to render them in the PDFs.