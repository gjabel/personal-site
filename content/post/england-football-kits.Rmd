...
I am loving the [magick](https://cran.r-project.org/web/packages/magick/) package at the moment. Reading through the [vignette](https://cran.r-project.org/web/packages/magick/vignettes/intro.html) I spotted the `image_morph()` function. In this post I experiment with the function to build the GIF below that shows the changes in the England football first kit over time, using images from the excellent [Historical Football Kits](http://www.historicalkits.co.uk/) website.
...
The final step was to bind together the set of images in an animated GIF with smooth transition images between each frame. To do this I used the `image_morph()` twice. First to repeat the same image so that the GIF would remain stable for a few frames (`kits_morph1` below). Then second to create a set of morphing images between successive kits (`kits_morph0` below). The full set of frames were stored in `kits_ani`

```{r, eval=FALSE}
kits_ani <- image_morph(c(kits$img[1], kits$img[1]), frames = 5)
...
I did have trouble creating a GIF's when I more frames for morphing images between successive kits, such as `frames = 10`. I could not consitently replicate the error, but I suspect it is something related to the memory size - my R session would freeze when either using `image_animate()` or `image_write()` on my `kits_ani` R object when it contained a large number of images.
